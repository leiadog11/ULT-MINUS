...
// INIT
unsafe extern "C" fn palutena_specials_charge_init(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn palutena_specials_charge_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if BLACKHOLE_CHARGE[ENTRY_ID] >= 120 && BLACKHOLE_CHARGE[ENTRY_ID] <= 121  { 
        ...
        let eff = EffectModule::req_on_joint(fighter.module_accessor, Hash40::new("sys_flash"), Hash40::new("hip"), &vector, &vector, 0.5, &vector, &vector, false, 0, 0, 0);
        EffectModule::set_rgb(fighter.module_accessor, eff.try_into().unwrap(), 0.5, 0.0, 0.5);
    }

    // FINISH
    if BLACKHOLE_CHARGE[ENTRY_ID] >= 360 {
        let mut vector = Vector3f{x:0.0,y:10.0,z:0.0};
        let eff = EffectModule::req_on_joint(fighter.module_accessor, Hash40::new("sys_flash"), Hash40::new("hip"), &vector, &vector, 1.2, &vector, &vector, false, 0, 0, 0);
        EffectModule::set_rgb(fighter.module_accessor, eff.try_into().unwrap(), 0.5, 0.0, 0.5);
        SoundModule::play_se(fighter.module_accessor, Hash40::new("se_item_deathscythe_swing_l"), true, false, false, false, enSEType(0));
        vector = Vector3f{x:0.0,y:8.65,z:1.0};
