...
unsafe extern "C" fn parasail_specialairhiglide_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    ControlModule::check_button_trigger(weapon.module_accessor, *CONTROL_PAD_BUTTON_GUARD) {
        weapon.change_status(WEAPON_LINK_PARASAIL_STATUS_KIND_SPECIAL_AIR_HI_UNEQUIP.into(), false.into());
        return 1.into();
    }
...
// PRE
unsafe extern "C" fn parasail_specialairhiunequip_pre(weapon: &mut L2CWeaponCommon) -> L2CValue {
    StatusModule::init_settings(
        weapon.module_accessor, 
        SituationKind(*SITUATION_KIND_AIR), 
        *WEAPON_KINETIC_TYPE_RESET, 
...
// MAIN
unsafe extern "C" fn parasail_specialairhiunequip_main(weapon: &mut L2CWeaponCommon) -> L2CValue {
    MotionModule::change_motion(weapon.module_accessor, Hash40::new("special_air_hi_unequip"), 0.0, 1.0, false, 0.0, false, false);

    weapon.fastshift(L2CValue::Ptr(parasail_specialairhiunequip_main_loop as *const () as _))
}

// MAIN LOOP
unsafe extern "C" fn parasail_specialairhiunequip_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    return 0.into();
}

// END
unsafe extern "C" fn parasail_specialairhiunequip_end(weapon: &mut L2CWeaponCommon) -> L2CValue {
    return 0.into();
}
...
pub fn install() {
    ...
    .status(Pre, WEAPON_LINK_PARASAIL_STATUS_KIND_SPECIAL_AIR_HI_UNEQUIP, parasail_specialairhiunequip_pre)
    .status(Main, WEAPON_LINK_PARASAIL_STATUS_KIND_SPECIAL_AIR_HI_UNEQUIP, parasail_specialairhiunequip_main)
    .status(End, WEAPON_LINK_PARASAIL_STATUS_KIND_SPECIAL_AIR_HI_UNEQUIP, parasail_specialairhiunequip_end)
