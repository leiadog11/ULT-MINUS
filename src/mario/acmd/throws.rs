...
unsafe extern "C" fn mario_throwb(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::ATTACK_IGNORE_THROW(agent, 0, 0, Hash40::new("hip"), 8.0, 50, 70, 0, 100, 4.0, -1.0, 6.0, 0.0, Some(-3.2), Some(13.0), Some(0.0), 1.0, 1.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_POS, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_KICK, *ATTACK_REGION_THROW);
    }
    ...
    if macros::is_excute(agent) {
        macros::CHECK_FINISH_CAMERA(agent, 14, 7);
...
unsafe extern "C" fn mario_throwf(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::CHECK_FINISH_CAMERA(agent, 14, 1);
        lua_bind::FighterCutInManager::set_throw_finish_zoom_rate(singletons::FighterCutInManager(), 1.5);
...
// GRAB
unsafe extern "C" fn mario_catch(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        ...
        macros::CATCH(agent, 1, Hash40::new("top"), 1.65, 0.0, 6.6, 2.35, Some(0.0), Some(6.6), Some(10.35), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_G);
    }
...
// DASH GRAB
unsafe extern "C" fn mario_catchdash(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        ...
        macros::CATCH(agent, 1, Hash40::new("top"), 1.3, 0.0, 6.6, 2.7, Some(0.0), Some(6.6), Some(11.7), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_G);
    }
    macros::game_CaptureCutCommon(agent);
    wait(agent.lua_state_agent, 2.0);
    if macros::is_excute(agent) {
...
// PIVOT GRAB
unsafe extern "C" fn mario_catchturn(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 9.0);
    if macros::is_excute(agent) {
        GrabModule::set_rebound(agent.module_accessor, true);
    }
    ...
    if macros::is_excute(agent) {
        ...
        macros::CATCH(agent, 1, Hash40::new("top"), 1.65, 0.0, 6.6, -2.35, Some(0.0), Some(6.6), Some(-16.35), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_G);
    }
    macros::game_CaptureCutCommon(agent);
    wait(agent.lua_state_agent, 2.0);
    if macros::is_excute(agent) {
...
pub fn install() {
    Agent::new("mario")
        ...
        .game_acmd("game_throwlw", mario_throwlw, Low)

        .game_acmd("game_throwhi", mario_throwhi, Low)

        .game_acmd("game_catchattack", mario_catchattack, Low)

        .game_acmd("game_catch", mario_catch, Low)

        .game_acmd("game_catchdash", mario_catchdash, Low)

        .game_acmd("game_catchturn", mario_catchturn, Low)
